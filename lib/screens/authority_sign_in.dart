...
class _AuthorityPhoneScreenState extends State<AuthorityPhoneScreen> {
  ...
  void initState() {
    ...
    _typer = Timer.periodic(const Duration(milliseconds: 75), (t) {
      ...
      i++;
      _ctrl.value = TextEditingValue(text: demo.substring(0, i), selection: TextSelection.collapsed(offset: i));
  ...
  Widget build(BuildContext context) {
    ...
    return DarkPage(
      ...
      body: ListView(padding: const EdgeInsets.symmetric(horizontal: 18), children: [
        const _Steps(1),
        const SizedBox(height: 20),
        Text("What's your phone number?", style: ft(22, w: 700)),
        const SizedBox(height: 10),
        Text("Use the number the Hostel Affairs office has on record for you. Hostel authorities don't need an Outlook ID.",
        ...
        Btn('Send code on SMS', onTap: _digits == 10
...
class _AuthorityCodeScreenState extends State<AuthorityCodeScreen> {
  ...
  void initState() {
    ...
    _timers.add(Timer(const Duration(milliseconds: 400), () {
      _timers.add(Timer.periodic(const Duration(milliseconds: 140), (t) {
                ...
                SizedBox(
                  ...
                  child: AnimatedSwitcher(
                    ...
                    transitionBuilder: (c, a) => FadeTransition(
                      opacity: a,
                      child: SlideTransition(position: Tween(begin: const Offset(0, 0.35), end: Offset.zero).animate(a), child: c),
                    ),
                    child: Text(
                      i < _filled ? code[i] : '-',
