...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final items = <_Item>[
      ...
      _Item('GATELOG', DateTime.now().subtract(const Duration(days: 1)), 'Gatelog entry approved', "Your 'To City' request for 6 September was approved."),
      _Item('CAB SHARING', DateTime.now().subtract(const Duration(days: 1, hours: 2)), 'Someone joined your ride', 'Rohit K. joined your 6:30 AM cab to Guwahati Airport.'),
    ...
    return DarkPage(
      ...
      body: Column(children: [
        ...
        Expanded(
          child: ListView(padding: const EdgeInsets.fromLTRB(16, 10, 16, 24), children: [
            for (final i in shown)
              Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: Panel(
                  onTap: i.report == null ? null : () => Navigator.push(context, MaterialPageRoute(builder: (_) => SightingDetailScreen(report: i.report!))),
            ...
            const SizedBox(height: 6),
            Text('Snake Watch alerts can be turned off in Profile › Notification preferences.', textAlign: TextAlign.center, style: ft(11.5, color: C.faint)),
          ]),
        ),
      ]),
    );
